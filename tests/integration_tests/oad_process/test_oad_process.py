...
def test_api(cleanup):
    ...
    # Run optim ---------------------------------------------------------------
    problem = api.optimize_problem(configuration_file_path, True, auto_scaling=True)
    ...
    assert_allclose(problem["data:geometry:wing:MAC:at25percent:x"], 16.55, atol=1e-2)
    assert_allclose(problem["data:weight:aircraft:MTOW"], 76606, atol=1)
    assert_allclose(problem["data:geometry:wing:area"], 125.83, atol=1e-2)
    assert_allclose(problem["data:geometry:vertical_tail:area"], 24.3, atol=2e-1)
    assert_allclose(problem["data:geometry:horizontal_tail:area"], 34.8, atol=2e-1)
    assert_allclose(problem["data:mission:sizing:fuel"], 20240, atol=1)
