...
def test_init_bundle_loader_from_scratch():
    """
    Tests that creating a Loader instance will start a correctly configured Pelix framework
    """
...
def test_init_bundle_loader_with_running_framework():
    """
    Tests that creating a Loader instance with an already running Pelix framework will work and enforce a correct
    ...
    # Creating a loader instance when existing framework misses pelix.ipopo.core bundle
    # It should be added to the framework a Loader initialization
    loader = Loader()
    assert framework is loader.framework
...
def test_get_service_references():
    ...
    service_refs = loader.get_service_references("hello.world", {"Prop1": 1, "Prop2": "says.hello"})
    assert len(service_refs) == 1
    ...
    service_refs = loader.get_service_references("hello.world", {"Prop2": "SAYS.HI"})
    assert len(service_refs) == 1
    greet_service = loader.context.get_service(service_refs[0])
    assert greet_service.hello() == "Hi, World!"

    # Existing service, case sensitivity
    service_refs = loader.get_service_references("hello.world", {"Prop2": "SAYS.HELLO"}, True)
    assert service_refs is None
    service_refs = loader.get_service_references("hello.world", {"Prop2": "Says.Hello"}, True)
    assert len(service_refs) == 1
