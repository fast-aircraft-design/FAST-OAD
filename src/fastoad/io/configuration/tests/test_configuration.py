...
def test_problem_definition_no_input_file(cleanup):
    ...
    with pytest.raises(ValidationError) as exc_info:
        conf.load(DATA_FOLDER_PATH / "missing_input_file.yml")
...
def test_problem_definition_no_output_file(cleanup):
    ...
    conf = FASTOADProblemConfigurator()
    with pytest.raises(ValidationError) as exc_info:
        conf.load(DATA_FOLDER_PATH / "missing_output_file.yml")
...
def test_problem_definition_no_model(cleanup):
    ...
    conf = FASTOADProblemConfigurator()
    with pytest.raises(ValidationError) as exc_info:
        conf.load(DATA_FOLDER_PATH / "missing_model.yml")
...
def test_problem_definition_incorrect_attribute(cleanup):
    clear_openmdao_registry()
    conf = FASTOADProblemConfigurator()
    conf.load(DATA_FOLDER_PATH / "invalid_attribute.yml")
...
def test_problem_definition_no_module_folder(cleanup):
    clear_openmdao_registry()
    conf = FASTOADProblemConfigurator()
    conf.load(DATA_FOLDER_PATH / "no_module_folder.yml")
...
def test_problem_definition_module_folder_as_one_string(cleanup):
    clear_openmdao_registry()
    conf = FASTOADProblemConfigurator()
    conf.load(Path(__file__).parent / "data" / "module_folder_as_one_string.yml")
    conf.get_problem()
...
def test_problem_definition_correct_configuration(cleanup):
    clear_openmdao_registry()
    conf = FASTOADProblemConfigurator()
    conf.load(DATA_FOLDER_PATH / "valid_sellar.yml")
...
def test_problem_definition_with_xml_ref(cleanup, caplog):
    ...
    clear_openmdao_registry()
    conf = FASTOADProblemConfigurator(DATA_FOLDER_PATH / "valid_sellar.yml")
    ...
    assert Path(problem.model_options["cycle.*"]["input_file"]) == DATA_FOLDER_PATH / "__init__.py"
    ...
    alt_conf = FASTOADProblemConfigurator(DATA_FOLDER_PATH / "valid_sellar_alternate.yml")
...
def test_problem_definition_with_xml_ref_with_indep(cleanup):
    ...
    clear_openmdao_registry()
    conf = FASTOADProblemConfigurator(DATA_FOLDER_PATH / "valid_sellar_with_indep.yml")

    result_folder_path = RESULTS_FOLDER_PATH.joinpath("problem_definition_with_xml_ref_with_indep")
    conf.input_file_path = result_folder_path / "inputs.xml"
    conf.output_file_path = result_folder_path / "outputs.xml"
    ref_input_data_path = DATA_FOLDER_PATH / "ref_inputs.xml"
    conf.write_needed_inputs(ref_input_data_path)
...
def test_problem_definition_with_xml_ref_run_optim(cleanup):
    ...
    result_folder_path = RESULTS_FOLDER_PATH.joinpath("problem_definition_with_xml_ref_run_optim")
    conf.input_file_path = result_folder_path / "inputs.xml"
    input_data = DATA_FOLDER_PATH / "ref_inputs.xml"
...
def test_set_optimization_definition(cleanup):
    ...
    """
    clear_openmdao_registry()
    reference_file = DATA_FOLDER_PATH / "valid_sellar.yml"
    editable_file = RESULTS_FOLDER_PATH / "editable_valid_sellar.yml"
...
def test_make_local(cleanup):
    ...
    for copy_models in [True, False]:
        ...
        target_folder = RESULTS_FOLDER_PATH / f"local_yml{'_with_models' if copy_models else ''}"
        ...
        assert filecmp.cmp(target_folder / "ref_inputs.xml", DATA_FOLDER_PATH / "ref_inputs.xml")
        assert filecmp.cmp(
        ...
        # Run -----------------------------------------------------
        clear_openmdao_registry()
        local_conf_1 = FASTOADProblemConfigurator(target_folder / "valid_sellar.yml")
        ...
        clear_openmdao_registry()
        local_conf_2 = FASTOADProblemConfigurator(target_folder / "valid_sellar.yml")
        ...
        if copy_models:
            ...
            clear_openmdao_registry()
            local_conf_3 = FASTOADProblemConfigurator(target_folder / "valid_sellar.yml")
