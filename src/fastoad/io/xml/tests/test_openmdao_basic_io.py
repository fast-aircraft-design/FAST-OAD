...
def _check_basic_ivc(ivc: IndepVarComp):
    ...
    assert_allclose(40.0, outputs["geometry:fuselage:length"].value)
    assert outputs["geometry:fuselage:length"].units == "m"

    assert_allclose(-42.0, outputs["constants"].value)
    assert outputs["constants"].units is None

    assert_allclose([1.0, 2.0, 3.0], outputs["constants:k1"].value)
    assert outputs["constants:k1"].units == "kg"

    assert_allclose([10.0, 20.0], outputs["constants:k2"].value)
    assert outputs["constants:k2"].units is None

    assert_allclose([100.0, 200.0, 300.0, 400.0], outputs["constants:k3"].value)
...
def test_basic_xml_read_and_write_from_ivc(cleanup):
    ...
    ivc.add_output("constants", val=[-42.0], desc="the answer")
