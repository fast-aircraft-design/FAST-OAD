...
def test_breguet_from_mtow():
    ...
    ivc.add_output('data:aerodynamics:aircraft:cruise:L_D_max', 16.)
    ...
    assert_allclose(problem['data:mission:sizing:ZFW'], 65617., rtol=1e-3)
    assert_allclose(problem['data:mission:sizing:fuel'], 8382., rtol=1e-3)
    ...
    assert_allclose(problem['data:mission:sizing:ZFW'], 62473., rtol=1e-3)
    assert_allclose(problem['data:mission:sizing:fuel'], 11526., rtol=1e-3)

    # Check consistency of other outputs
    assert_allclose(problem['data:mission:sizing:fuel'],
                    problem['data:mission:sizing:trip:fuel']
                    + problem['data:mission:sizing:fuel_reserve']
                    , rtol=1e-3)
    assert_allclose(problem['data:mission:sizing:trip:fuel'],
...
def test_breguet_from_mtow_with_rubber_engine():
    ...
    assert_allclose(problem['data:mission:sizing:ZFW'], 65076., atol=1)
    assert_allclose(problem['data:mission:sizing:fuel'], 8924., atol=1)
...
def test_breguet_from_owe():
    ...
    assert_allclose(problem['data:weight:aircraft:MTOW'], 74000., rtol=1e-3)
    assert_allclose(problem['data:mission:sizing:ZFW'], 65617., rtol=1e-3)
    assert_allclose(problem['data:mission:sizing:fuel'], 8382., rtol=1e-3)
...
def test_breguet_from_owe_with_rubber_engine():
    ...
    assert_allclose(problem['data:weight:aircraft:MTOW'], 74000., atol=10)
    assert_allclose(problem['data:mission:sizing:ZFW'], 65076., atol=1)
    assert_allclose(problem['data:mission:sizing:fuel'], 8924., atol=1)
