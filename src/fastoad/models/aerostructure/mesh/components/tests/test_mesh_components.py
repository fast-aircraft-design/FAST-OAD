...
def test_wing_chords():
    ...
    chord = problem["data:aerostructural:aerodynamic:wing:local_chords"]

    assert chord[0] == approx(6.20243, abs=1e-5)  # Check root chord
    assert chord[2] == approx(4.90715, abs=1e-5)  # Check intermediate inner chord
    ...
    assert chord[15] == approx(4.90715, abs=1e-5)  # Check symmetry intermediate inner chord
...
def test_htail_chords():
    ...
    problem = run_system(group, ivc)
    chord = problem["data:aerostructural:aerodynamic:horizontal_tail:local_chords"]
...
def test_vtail_chords():
    ...
    problem = run_system(group, ivc)
    chord = problem["data:aerostructural:aerodynamic:vertical_tail:local_chords"]
...
def test_fuselage_chords():
    ...
    problem = run_system(group, ivc)
    chord = problem["data:aerostructural:aerodynamic:fuselage:local_chords"]
