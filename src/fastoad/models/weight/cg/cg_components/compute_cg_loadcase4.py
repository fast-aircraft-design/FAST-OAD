...
class ComputeCGLoadCase4(AbstractComputeCGLoadCase):
    # TODO: Document equations. Cite sources
    """ Center of gravity estimation for load case 3 """

    def setup(self):
        self.options["case_number"] = 4
        return super().setup()
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        outputs[self.output_name] = self.compute_cg_ratio(
            inputs,
            weight_per_pax=90.0,
            weight_front_fret=inputs["data:TLAR:NPAX"] * 10.0,
            weight_rear_fret=inputs["data:TLAR:NPAX"] * 30.0,
        )
