...
def stability_diagram_plot(
    ...
    cl_max_wing = variables["data:aerodynamics:aircraft:landing:CL_max"].value[0]
    ...
    x_ht_form_wing = variables["data:geometry:horizontal_tail:MAC:at25percent:x:from_wingMAC25"].value[0]
    ...
    fuselage_radius = 0.25 * (variables["data:geometry:fuselage:maximum_height"].value[0] +
    ...
    v2 = pi * fuselage_radius ** 2 * (fuselage_length - fuselage_rear_length - fuselage_front_length)
    ...
    ht_effectiveness = 1 - cl_alpha_wing * 8 / (pi ** 3 * aspect_ratio_wing) * (1 + 1 / np.cos(beta))
    numerator = x_aerodynamic_center * cl_alpha_wing + surface_ratio * x_ht * cl_alpha_ht * ht_effectiveness - 2.0 * v / area_wing
    ...
    def epsilon(cl_wing):
        return -8 * (cl_wing) / (pi ** 3 * aspect_ratio_wing) * (1 + 1 / np.cos(beta))

    def lift_equilibrium(alpha, surface_ratio, mass):
        v_stall = 2 * mass * 9.81 / (rho * area_wing * cl_max_clean_wing * area_wing)

        left_member = mass * 9.81 / (0.5 * rho * (1.3 * v_stall) ** 2 * area_wing)

        cl_w = cl0_wing + alpha * cl_alpha_wing + cl_delta_flaps
        epsilon_ht = epsilon(cl_w)
        cl_ht = cl_alpha_ht * (alpha + epsilon_ht + ths_deportation)

        right_member = cl_w + surface_ratio * cl_ht
    ...
    # Moment equilibrium
    numerator = -cm0_wing * mac + x_aerodynamic_center * cl_wing + surface_ratio * x_ht * cl_horizontal_tail - 2 * alpha * v / area_wing  # ac near pc
    ...
    delta_x_cg = [0,0]
    delta_x_cg[0] = np.interp(actual_surface_ratio,surface_ratio,x_cg_front_percentage)
    delta_x_cg[1] = np.interp(actual_surface_ratio,surface_ratio,x_cg_rear_percentage_minus_5perc)

    # 2) Negative_ressource :
    v_mo = 463.0 / 3.6  # Speed see EASA CS25
    q = 9.81 / (2.0 * v_mo)  # rad/s : rotation speed

    def lift_equilibrium2(alpha, surface_ratio, mass):
        left_member = mass * 9.81 / (0.5 * rho * v_mo ** 2 * area_wing)

        cl_w = cl0_wing + alpha * cl_alpha_wing + cl_delta_flaps + q * mac / v_mo
        epsilon_ht = epsilon(cl_w)
        cl_ht = cl_alpha_ht * (alpha + epsilon_ht + ths_deportation + q * mac_ht / v_mo)
    ...
    actual_string = "$\Delta X_{cg}$ : %9.3f" % delta_x_cg_dif + "%"
    print(actual_string)

    scatter_actual = go.Scatter(
        x=delta_x_cg*100,
        y=[actual_surface_ratio,actual_surface_ratio]*100,
        line=dict(color="green"),
        mode="lines",
        name= actual_string,#"$\Delta X_{cg}$ %.1f: "  ,
    )
    ...
    fig.update_layout(
        title_text="Stability diagram : " + name,
        title_x=0.5,
